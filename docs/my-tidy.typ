...
#let show-parameter-values(values) = {
  ...
  for (name, info) in values {
    if info.type == "string" { items.push(raw("\"" + name + "\"", lang: "typc")) } else {
      items.push(raw(name, lang: "typc"))
    }
...
#let show-example-blocks(
  ...
  layout(size => style(styles => {
    ...
    let height = if dir.axis() == "vertical" { auto } else { measure(arrangement(width: size.width), styles).height }
